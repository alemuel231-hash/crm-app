...
export default function CompaniesPage() {
  ...
  const handleOpenModal = (item?: Companies) => {
    if (item) {
      setEditingItem(item);
      setFormData({ ...item });
    } else {
      setEditingItem(null);
      setFormData({ companyName: '', industry: '', website: '', status: '',  });
    }
    setShowModal(true);
  };
  ...
  const filteredItems = items.filter(i => 
    i.companyName?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)] flex items-center gap-2">
              <Building className="text-[#39379e]" size={24} /> Companies
            </h1>
            <p className="text-xs text-[var(--text-muted)] mt-1">Manage your companies.</p>
          </div>
          <button 
          ...
          <div className="overflow-x-auto">
            <table className="orbit-table w-full">
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">Loading companies...</td>
                  </tr>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Company Name</label>
                  <input required type="text" value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Industry</label>
                  <input required type="text" value={formData.industry} onChange={e => setFormData({...formData, industry: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Website</label>
                  <input required type="text" value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
