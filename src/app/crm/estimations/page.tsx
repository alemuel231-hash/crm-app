...
import { Search, Plus, Edit, Trash2, X, Calculator } from 'lucide-react';
...
export default function EstimationsPage() {
  const [items, setItems] = useState<Estimations[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<Estimations | null>(null);
  const [formData, setFormData] = useState({
    estimateId: '', client: '', amount: 0, status: '', 
  });

  const fetchItems = async () => {
    try {
      const res = await fetch('/api/estimations').then(r => r.json());
  ...
  const handleOpenModal = (item?: Estimations) => {
    if (item) {
      setEditingItem(item);
      setFormData({ ...item });
    } else {
      setEditingItem(null);
      setFormData({ estimateId: '', client: '', amount: 0, status: '',  });
    }
    setShowModal(true);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/estimations?id=${id}`, { method: 'DELETE' });
      fetchItems();
    } catch (error) {
      console.error(error);
    }
  ...
  const filteredItems = items.filter(i => 
    i.estimateId?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)] flex items-center gap-2">
              <Calculator className="text-[#39379e]" size={24} /> Estimations
            </h1>
            <p className="text-xs text-[var(--text-muted)] mt-1">Manage your estimations.</p>
          </div>
          <button 
        ...
        <div className="orbit-card bg-[var(--card-bg)] border border-[var(--border-color)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="orbit-table w-full">
              <thead>
                <tr>
                  <th className="text-left">Estimate ID</th>
                  <th className="text-left">Client</th>
                  <th className="text-left">Amount (GHS)</th>
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">Loading estimations...</td>
                  </tr>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Estimate ID</label>
                  <input required type="text" value={formData.estimateId} onChange={e => setFormData({...formData, estimateId: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Client</label>
                  <input required type="text" value={formData.client} onChange={e => setFormData({...formData, client: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Amount (GHS)</label>
                  <input required type="number" value={formData.amount} onChange={e => setFormData({...formData, amount: Number(e.target.value)})} className="orbit-input w-full" />
                </div>
                <div>
