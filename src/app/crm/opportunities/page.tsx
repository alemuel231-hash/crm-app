...
import { Search, Plus, Edit, Trash2, X, Lightbulb } from 'lucide-react';
...
export default function OpportunitiesPage() {
  ...
  const [editingItem, setEditingItem] = useState<Opportunities | null>(null);
  const [formData, setFormData] = useState({
    oppName: '', value: 0, stage: '', 
  });

  const fetchItems = async () => {
    try {
      const res = await fetch('/api/opportunities').then(r => r.json());
  ...
  const handleOpenModal = (item?: Opportunities) => {
    if (item) {
      setEditingItem(item);
      setFormData({ ...item });
    } else {
      setEditingItem(null);
      setFormData({ oppName: '', value: 0, stage: '',  });
    }
    setShowModal(true);
  };
  ...
  const filteredItems = items.filter(i => 
    i.oppName?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)] flex items-center gap-2">
              <Lightbulb className="text-[#39379e]" size={24} /> Opportunities
            </h1>
            <p className="text-xs text-[var(--text-muted)] mt-1">Manage your opportunities.</p>
          </div>
          <button 
        ...
        <div className="orbit-card bg-[var(--card-bg)] border border-[var(--border-color)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="orbit-table w-full">
              <thead>
                <tr>
                  <th className="text-left">Opportunity Name</th>
                  <th className="text-left">Value (GHS)</th>
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {loading ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">Loading opportunities...</td>
                  </tr>
                ) : filteredItems.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">No records found.</td>
                  </tr>
                ) : (
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Opportunity Name</label>
                  <input required type="text" value={formData.oppName} onChange={e => setFormData({...formData, oppName: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Value (GHS)</label>
                  <input required type="number" value={formData.value} onChange={e => setFormData({...formData, value: Number(e.target.value)})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Stage</label>
                  <input required type="text" value={formData.stage} onChange={e => setFormData({...formData, stage: e.target.value})} className="orbit-input w-full" />
                </div>
