...
import { Search, Plus, Edit, Trash2, X, FileSignature } from 'lucide-react';
...
export default function ProposalsPage() {
  ...
  const [editingItem, setEditingItem] = useState<Proposals | null>(null);
  const [formData, setFormData] = useState({
    proposalId: '', client: '', value: 0, status: '', 
  });
  ...
  const handleOpenModal = (item?: Proposals) => {
    if (item) {
      setEditingItem(item);
      setFormData({ ...item });
    } else {
      setEditingItem(null);
      setFormData({ proposalId: '', client: '', value: 0, status: '',  });
    }
    setShowModal(true);
  };
  ...
  const filteredItems = items.filter(i => 
    i.proposalId?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)] flex items-center gap-2">
              <FileSignature className="text-[#39379e]" size={24} /> Proposals
            </h1>
            <p className="text-xs text-[var(--text-muted)] mt-1">Manage your proposals.</p>
          </div>
          <button 
        ...
        <div className="orbit-card bg-[var(--card-bg)] border border-[var(--border-color)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="orbit-table w-full">
              <thead>
                <tr>
                  <th className="text-left">Proposal ID</th>
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">Loading proposals...</td>
                  </tr>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Proposal ID</label>
                  <input required type="text" value={formData.proposalId} onChange={e => setFormData({...formData, proposalId: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
