...
import { Search, Plus, Edit, Trash2, X, Banknote } from 'lucide-react';
...
export default function PayoutsPage() {
  ...
  const [editingItem, setEditingItem] = useState<Payouts | null>(null);
  const [formData, setFormData] = useState({
    payoutId: '', driver: '', amount: 0, status: '', 
  });

  const fetchItems = async () => {
    try {
      const res = await fetch('/api/payout').then(r => r.json());
  ...
  const handleOpenModal = (item?: Payouts) => {
    if (item) {
      setEditingItem(item);
      setFormData({ ...item });
    } else {
      setEditingItem(null);
      setFormData({ payoutId: '', driver: '', amount: 0, status: '',  });
    }
    setShowModal(true);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/payout?id=${id}`, { method: 'DELETE' });
      fetchItems();
    } catch (error) {
      console.error(error);
    }
  ...
  const filteredItems = items.filter(i => 
    i.payoutId?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)] flex items-center gap-2">
              <Banknote className="text-[#39379e]" size={24} /> Payouts
            </h1>
            <p className="text-xs text-[var(--text-muted)] mt-1">Manage your payouts.</p>
          </div>
          <button 
          ...
          <div className="overflow-x-auto">
            <table className="orbit-table w-full">
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">Loading payouts...</td>
                  </tr>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Payout ID</label>
                  <input required type="text" value={formData.payoutId} onChange={e => setFormData({...formData, payoutId: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Driver</label>
                  <input required type="text" value={formData.driver} onChange={e => setFormData({...formData, driver: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
