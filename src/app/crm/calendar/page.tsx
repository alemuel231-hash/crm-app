...
export default function CalendarPage() {
  ...
  const [editingItem, setEditingItem] = useState<Calendar | null>(null);
  ...
  const handleOpenModal = (item?: Calendar) => {
    if (item) {
      setEditingItem(item);
      setFormData({ ...item });
    } else {
      setEditingItem(null);
      setFormData({ eventName: '', date: '', time: '', location: '',  });
    }
    setShowModal(true);
  };
  ...
  const filteredItems = items.filter(i => 
    i.eventName?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)] flex items-center gap-2">
              <Calendar className="text-[#39379e]" size={24} /> Calendar
            </h1>
            <p className="text-xs text-[var(--text-muted)] mt-1">Manage your calendar.</p>
          </div>
          <button 
          ...
          <div className="overflow-x-auto">
            <table className="orbit-table w-full">
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-[var(--text-muted)] text-xs font-semibold">Loading calendar...</td>
                  </tr>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Event Name</label>
                  <input required type="text" value={formData.eventName} onChange={e => setFormData({...formData, eventName: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Time</label>
                  <input required type="text" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} className="orbit-input w-full" />
                </div>
                <div>
                  <label className="block text-[10px] text-[var(--text-muted)] uppercase mb-1">Location</label>
                  <input required type="text" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="orbit-input w-full" />
                </div>
